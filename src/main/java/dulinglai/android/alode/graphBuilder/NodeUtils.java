...
public class NodeUtils {
     ...
     */
    public static List<String> processIntentFilter(AXmlNode node, String type){
        ...
        for (AXmlNode ifNode : ifNodes){
            for (AXmlNode subnode : ifNode.getChildrenWithTag(type)){
    ...
    public static String processAuthorities(AXmlNode node){
        ...
        if (attrAuthorities!=null)
            return (String)attrAuthorities.getValue();
        else
            return null;
    }
    ...
    public static EditWidgetNode createEditWidgetNode(AXmlNode node){
        int resourceId = -1;
        String text = "";
